...
plt.tight_layout()
plt.savefig(os.path.join(INTERP_DIR, "val_encoder_importance.png"), dpi=150)
plt.close()

plt.figure(figsize=(8, 4))
dec_val.head(12).plot.bar(x="feature", y="importance", legend=False, title="Decoder Variable Importance (VAL)")
plt.tight_layout()
plt.savefig(os.path.join(INTERP_DIR, "val_decoder_importance.png"), dpi=150)
plt.close()

def safe_plot(idx: int, raw, interp, prefix: str):
    try:
